...
function EventDetail({ event, onClose }) {
  // Al montar, scrollea al panel de detalle automáticamente
  useEffect(() => { 
    if (event) {
      // Pequeño timeout para asegurar que el DOM se renderizó antes de scrollear
      setTimeout(() => {
        scrollToElement("detail-panel-anchor"); 
      }, 100);
    }
  }, [event]);
  ...
  return (
    <div className="nt-detail-panel" id="detail-panel-anchor">
      ...
      <div className="nt-detail-grid">
          ...
          <div className="nt-grid-bottom">
             {event.fullDetails?.map((section, idx) => (
               <div key={idx} className="nt-detail-card">
                 ...
                 {/* Listas de ítems */}
                 {section.list && (
                    <ul className="nt-list-custom">
                        {section.list.map((item, i) => <li key={i}>{item}</li>)}
                    </ul>
                 )}

                 {/* Sub-secciones opcionales */}
        ...
        {/* COLUMNA DERECHA: Datos Rápidos y Conversión (Sticky) */}
        <aside className="nt-detail-right">
          {/* Caja Meta (Fecha y Lugar) */}
          ...
          {/* Sección de Beneficios y Premios */}
          ...
          {/* Reglamento */}
          {event.reglamentoLink && (
            <span className="nt-reglamento">Descargar Reglamento</span>
          )}
        </aside>
      </div>
      
      {/* Botón Cerrar Inferior (Visible solo en móvil para UX) */}
...
export default function Noticias() {
  ...
  return (
    ...
    <div className="nt-ui-overlay"></div>

    {/* ERROR ENCONTRADO: Faltaba agregar "anim-fade-in" aquí abajo */}
    <div className="nt-container anim-fade-in">
